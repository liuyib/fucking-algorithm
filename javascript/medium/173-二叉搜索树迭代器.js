...
BSTIterator.prototype.next = function () {
  while (this.nodes.length) {
    const [node, color] = this.nodes.pop();

    if (color === WHITE) {
      if (node.right) this.nodes.push([node.right, WHITE]);
      this.nodes.push([node, GRAY]);
      if (node.left) this.nodes.push([node.left, WHITE]);
    } else {
      return node.val;
    }
  }
